 ...
 */
const { onPaneReady, onNodeDragStop, onConnect, addEdges, toObject, findNode, addNodes, project, vueFlowRef, getSelectedElements } = useVueFlow();
...
onKeyStroke('Backspace', checkDeleteNode);
onKeyStroke('Delete', checkDeleteNode);

const { ctrl, z, shift, y } = useMagicKeys();
watchEffect(() => {
...
function onDrop (event: DragEvent) {
  ...
  if (!nodeStr) return;
  const node: NodeInterface = JSON.parse(nodeStr);
...
onConnect((params) => {
  // 防止输入连上输入、输出连上输出
  if(checkInvalidConnect(params)) return;
  addEdges([{ ...params, animated: true, label: 'connect', style: { stroke: 'orange' }, labelBgStyle: { fill: 'orange' }, }]);
  commit()
})
...
<template>
  <div class="dndflow" @drop="onDrop">
    ...
    <VueFlow v-model="elements" :class="{ dark }" class="basicflow" :default-viewport="{ zoom: 0.5, x: 0, y: 0 }"
      :min-zoom="0.2" :max-zoom="4" @dragover="onDragOver">
      <Background :pattern-color="dark ? '#FFFFFB' : '#aaa'" :gap="8" />
      <MiniMap />
      <Controls />

      <Panel :position="PanelPosition.TopRight" class="controls">
        <button style="background-color: #113285; color: white" title="Undo" @click="handleUndo">
        ...
        <button style="background-color: #6f3381; transform: rotateY(180deg);" title="Redo" @click="handleRedo">
          <svg width="16" height="16" viewBox="0 0 32 32">
            <path fill="#FFFFFB"
              d="M18 28A12 12 0 1 0 6 16v6.2l-3.6-3.6L1 20l6 6l6-6l-1.4-1.4L8 22.2V16a10 10 0 1 1 10 10Z" />
          </svg>
